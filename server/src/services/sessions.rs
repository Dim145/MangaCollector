...
//!
//! tower-sessions stores its own opaque rows in `tower_sessions`;
//! we keep a parallel index in `user_session_meta` so the SPA can
//! list and revoke per-user sessions without reverse-engineering
//! the upstream BYTEA encoding. The two tables are bound by FK +
//! ON DELETE CASCADE: revoking one cascades to the other.
...
pub async fn record_login(
    ...
    let trimmed_ua = user_agent
        .map(|ua| {
            let s = ua.trim();
            s.chars().take(UA_MAX_LEN).collect::<String>()
        })
        .filter(|s| !s.is_empty());
    ...
    let _ = SessionMetaEntity::insert(model)
        .on_conflict(
            OnConflict::column(session_meta::Column::SessionId)
                .update_columns([
                    session_meta::Column::UserId,
